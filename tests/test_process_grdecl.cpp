...
BOOST_AUTO_TEST_CASE(Single_Fault_Connection)
{
    ...
    // Face vertices (offsets)
    {
        ...
        BOOST_CHECK_EQUAL_COLLECTIONS(out.face_ptr, out.face_ptr + out.number_of_faces + 1,
                                      expect.begin(), expect.end());
    }
    ...
    // Face vertices (vertex IDs)
    {
        ...
        BOOST_REQUIRE_EQUAL(expect.size(), static_cast<std::size_t>(out.face_ptr[out.number_of_faces]));

        BOOST_CHECK_EQUAL_COLLECTIONS(out.face_nodes, out.face_nodes + out.face_ptr[out.number_of_faces],
                                      expect.begin(), expect.end());
    }
}
