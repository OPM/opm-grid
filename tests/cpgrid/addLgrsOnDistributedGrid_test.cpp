                           ...
                           const std::vector<std::string>& lgr_name_vec)
{
    ...
    for (long unsigned int level = 1; level < startIJK_vec.size() +1; ++level) // only 1 when there is only 1 patch
    {
        ...
        if (!(data[level] -> global_cell_.empty()))
        {
            ...
            BOOST_CHECK( *itMin >= 0); // When the LGR is not fully interior, *itMin might be > 0.
...
/*BOOST_AUTO_TEST_CASE(distributed_in_all_ranks_lgr)
{
    ...
    if(grid.comm().size()>1)
    {
        ...
        // Max global id from level 0 = 115 (36+80 = 116 level zero entities)
        // Max global id for the leaf grid view = 115 + 64 + 98 = 277.
        // Total global ids points leaf = 80 + 98 = 178.
        grid.addLgrsUpdateLeafView(cells_per_dim_vec, startIJK_vec, endIJK_vec, lgr_name_vec);
...
BOOST_AUTO_TEST_CASE(distributed_lgrs)
{
    // Create a grid
    Dune::CpGrid grid;
    const std::array<double, 3> cell_sizes = {1.0, 1.0, 1.0};
    ...
    if(grid.comm().size()>1)
    {
        ...
        const std::vector<std::array<int,3>> startIJK_vec = {{1,0,0}, {1,1,2}};
        const std::vector<std::array<int,3>> endIJK_vec = {{3,2,1}, {3,3,3}};
        const std::vector<std::string> lgr_name_vec = {"LGR1", "LGR2"};
        // LGR1 element indices = {1,2,5,6} (1,5 in rank 0; 2,6 in rank 2). 
        // LGR2 element indices = {29,30,33,34} (29,33 in rank 1; 30,34 in rank 3).
...
BOOST_AUTO_TEST_CASE(three_fully_interior_lgrs)
{
    // Create a grid
    Dune::CpGrid grid;
    const std::array<double, 3> cell_sizes = {1.0, 1.0, 1.0};
...
BOOST_AUTO_TEST_CASE(atLeastOneFullyInteriorLgr_per_process)
{
    // Create a grid
    Dune::CpGrid grid;
    const std::array<double, 3> cell_sizes = {1.0, 1.0, 1.0};
