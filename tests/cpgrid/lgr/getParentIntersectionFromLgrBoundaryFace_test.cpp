...
std::set<int> checkBothOrientations(const Dune::CpGrid& grid,
                                    int indexInInsideFromCoarse,
                                    int indexInInsideFromRefined)
{
        ...
        for (const auto& intersection : Dune::intersections(grid.leafGridView(), element)) {
            if (intersection.neighbor() && (intersection.inside().level() != intersection.outside().level())) {
                ...
                if (intersection.inside().level() == 0)
                    BOOST_CHECK_EQUAL(parentIntersection.indexInInside(), indexInInsideFromCoarse);
                else
                    BOOST_CHECK_EQUAL(parentIntersection.indexInInside(), indexInInsideFromRefined);
            }
        }
    }
...
PORO
  2*0.15 /
    ...
    const std::set<int> parentIntersectionToPoint = checkBothOrientations(grid,
                                                                          /* indexInInsideromCoarse = */ 0,
                                                                          /* indexInInsideFromRefined = */ 1);
...
PORO
  2*0.15 /
    ...
    const auto parentIntersectionToPoint = checkBothOrientations(grid,
                                                                 /* indexInInsideromCoarse = */ 2,
                                                                 /* indexInInsideFromRefined = */ 3);
    BOOST_CHECK(parentIntersectionToPoint == expectedCornerIndices);
}
...
PORO
  2*0.15 /
    ...
    const auto parentIntersectionToPoint = checkBothOrientations(grid,
                                                                 /* indexInInsideromCoarse = */ 4,
                                                                 /* indexInInsideFromRefined = */ 5);
    BOOST_CHECK(parentIntersectionToPoint == expectedCornerIndices);
}
