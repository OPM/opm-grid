                           ...
                           const std::vector<std::string>& lgr_name_vec)
{
    ...
    auto& distributed_data = coarse_grid.distributed_data_;
    ...
    try
    {
        ...
        BOOST_CHECK( distributed_data[0]->child_to_parent_cells_.empty());
        BOOST_CHECK(coarse_grid.lgr_names_["GLOBAL"] == 0);
        const auto& all_parent_cell_indices = distributed_data[0]->getPatchesCells(startIJK_vec, endIJK_vec);
        ...
        for (long unsigned int level = 1; level < startIJK_vec.size() +1; ++level) // only 1 when there is only 1 patch
        {
            check_refinedPatch_grid(cells_per_dim_vec[level-1], startIJK_vec[level-1], endIJK_vec[level-1],
                                    (*data[level-1]).geometry_.template geomVector<0>(),
                                    (*data[level-1]).geometry_.template geomVector<1>(),
                                    (*data[level-1]).geometry_.template geomVector<3>());
            BOOST_CHECK( (*data[level-1]).parent_to_children_cells_.empty());
            BOOST_CHECK(coarse_grid.lgr_names_[lgr_name_vec[level-1]] == static_cast<int>(level));

            const auto& patch_cells = distributed_data[0]->getPatchCells(startIJK_vec[level-1], endIJK_vec[level-1]);

            std::cout<< " okay? " << std::endl;
            
            // GLOBAL grid
            for (int cell = 0; cell <  distributed_data[0]-> size(0); ++cell)
            {
                Dune::cpgrid::Entity<0> entity = Dune::cpgrid::Entity<0>(*distributed_data[0], cell, true);
