...
void lookup_check(const Dune::CpGrid& grid)
{
    ...
    for (const auto& elem : elements(leaf_view)) {
        ...
        levelCartMapp.cartesianCoordinateLevel(elem.getOrigin().index(), ijkLevel0, 0);
        BOOST_CHECK(ijk == ijkLevel0);
        // Throw for level < 0 or level > maxLevel()
        std::array<int,3> ijkThrow;
        BOOST_CHECK_THROW(levelCartMapp.cartesianCoordinateLevel(elem.index(), ijkThrow, -3), std::invalid_argument);
        BOOST_CHECK_THROW(levelCartMapp.cartesianCoordinateLevel(elem.index(), ijkThrow, grid.maxLevel() + 1), std::invalid_argument);
        ...
        if (elem.level())
        {
            ...
            levelCartMapp.cartesianCoordinateLevel(idxOnLevel, ijkLevel, elem.level());
