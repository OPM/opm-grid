...
namespace Dune
{
    ...
    template <int dim>
    array<double, 3> ReservoirPropertyCapillaryAnisotropicRelperm<dim>::computeSingleRockCflFactors(int rock) const
    {
        const int N = 257;
	...
	for (int direction = 0; direction < dim; ++direction) {
        ...
        array<double, 3> retval = {{ 1.0/max_der1, 1.0/max_derg, min_ffg }};
        return retval;
    }
    ...
    void ReservoirPropertyCapillaryAnisotropicRelperm<dim>::computeCflFactors()
    {
        if (Super::rock_.empty()) {
            array<double, 3> fac = computeSingleRockCflFactors(-1);
            ...
            for (int r = 0; r < int(Super::rock_.size()); ++r) {
                array<double, 3> fac = computeSingleRockCflFactors(r);
