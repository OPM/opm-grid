                                                ...
                                                const std::vector<std::string>& lgr_name_vec) const
{
    ...
    const auto& size = cells_per_dim_vec.size();
    ...
    for (std::size_t shiftedLevel = 0; shiftedLevel < size; ++shiftedLevel)
    {
        bool undesiredSubdivisions = (cells_per_dim_vec[shiftedLevel][0] == 1) && (cells_per_dim_vec[shiftedLevel][1]==1) && (cells_per_dim_vec[shiftedLevel][2]==1);
        if (!undesiredSubdivisions){
            filtered_cells_per_dim_vec.push_back(cells_per_dim_vec[shiftedLevel]);
            filtered_startIJK_vec.push_back(startIJK_vec[shiftedLevel]);
            filtered_endIJK_vec.push_back(endIJK_vec[shiftedLevel]);
            filtered_lgr_name_vec.push_back(lgr_name_vec[shiftedLevel]);
        }
        else {
            Opm::OpmLog::warning("Subdivions equal to 1 in all directions do not generate "+lgr_name_vec[shiftedLevel]+"\n");
        }
    }
