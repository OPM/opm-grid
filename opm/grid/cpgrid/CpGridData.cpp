...
{
    if (refCount == -1) {
        if (throwOnFailure) OPM_THROW(std::logic_error, "Coarsening is not supported yet.");
    ...
    if (hasNNCs({element.index()}) && (refCount == 1)) {
        if (throwOnFailure) OPM_THROW(std::logic_error, "Refinement of cells with face representing an NNC is not supported yet.");
        return false;
    }
