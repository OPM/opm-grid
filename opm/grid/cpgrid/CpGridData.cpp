...
CpGridData::refineSingleCell(const std::array<int,3>& cells_per_dim, int parent_idx,
                             std::vector<std::vector<std::pair<int, std::vector<int>>>>& faceInMarkedElemAndRefinedFaces) const
{
    ...
    parentCellGeom.refineCellifiedPatch(cells_per_dim, cellRefGeomData.geometries, cellRefGeomData.cell_to_point,
                                        cellRefGeomData.cell_to_face, cellRefGeomData.face_to_point,
                                        cellRefGeomData.face_to_cell, cellRefGeomData.face_tags,
                                        cellRefGeomData.face_normals, /* block/patch-dimensions = */ {1,1,1},
    ...
    cellRefinementBoundaryInfo.parentCellhasSingleFacePerType = Opm::Lgr::hasAtMostOneFacePerGroup(classifiedParentCellFaces);
    // In cellRefGrid, corner indices look like: (j*(cells_per_dim[0]+1)*(cells_per_dim[2]+1)) + (i*(cells_per_dim[2]+1)) + k
    ...
    cellRefinementBoundaryInfo.parentVertexIdx_to_boundaryRefinedVertexIdx = {
    ...
    if(cellRefinementBoundaryInfo.parentCellhasSingleFacePerType) {
        ...
        Opm::Lgr::populateBoundaryRefVertexCoincidesWithParentVertex(cellRefGrid_ptr->size(3), cellRefinementBoundaryInfo);
        ...
        Opm::Lgr::makeCellRefinementParentFaceAware(classifiedParentCellFaces,
                                                    /* parentGrid */ *this,
                                                    parentCellElem,
                                                    cellRefGrid,
