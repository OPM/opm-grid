...
public:
    ...
    /// \param clip_z if true, the grid will be clipped so that the top and bottom will be planar.
    /// \param poreVolume pore volumes for use in MINPV processing, if asked for in deck
    void processEclipseFormat(const Opm::EclipseGrid* ecl_grid, const Opm::Deck& deck,
                              const Opm::AquiferConfig& aquifer, bool periodic_extension, bool turn_normals = false, bool clip_z = false,
    ...
    void processEclipseFormat(const Opm::EclipseGrid& ecl_grid, const Opm::Deck& deck, const grdecl& input_data, const std::array<std::set<std::pair<int, int>>, 2>& nnc,
                              const Opm::AquiferConfig& aquifer, double z_tolerance, bool remove_ij_boundary, bool turn_normals = false);
