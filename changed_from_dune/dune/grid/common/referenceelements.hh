...
namespace Dune
{
  ...
  /**
     @addtogroup GridReferenceElements Old Dune Reference Elements
     @ingroup Grid
...
//               pos[(subsizes[0][0][dim-1])-1][1][j]=(pos[0][dim][j]+pos[1][2][j])/2;
 ...
 // subentity indices - in agreement with UG description
  ...
  /***********************************************************
   * Our new 'singular' type
   ***********************************************************/


  //! This is the reference singular without virtual functions
  template<typename ctype, int dim>
  class ReferenceSingular
  {
  public:
    enum { MAXE = 1 }; // maximum number of entities per codim
   ...
   //! maps local coordinate on subentity i of codim cdim onto reference element coordinates
    ...
    GeometryType type (int i, int c) const
    {
        return GeometryType(GeometryType::singular, dim-c);	  
    }
  ...
  private:
      FieldVector<ctype,dim> pos;
  };


  //! Make the reference Singular accessible as a container
  template<typename ctype, int dim>
  class ReferenceSingularContainer
  {
  public:
    //! export type elements in the container
    typedef ReferenceSingular<ctype,dim> value_type;
    ...
    const value_type& operator() (GeometryType type) const
    {
        if (type.isSingular())
	    return sing;
	DUNE_THROW(RangeError, "expected a singular!");
    }

  private:
      ReferenceSingular<ctype,dim> sing;
  };
  ...
  private:
    ...
    ReferenceElementWrapper<ReferenceSingular<ctype,dim> > singulars;
  };
  ...
  struct ReferenceElements {
    ...
    static ReferenceSimplexContainer<ctype,dim> simplices;
      static ReferenceSingularContainer<ctype,dim> singulars;
